  ...
  (
    {
      ...
      onLoginClick,
      onCtaClick,
      scrolled = false, // Default to false
      ...props
    },
    ref
  ) => {
    ...
    return (
      <header
        ref={combinedRef}
        className={cn(
          'sticky top-0 z-50 w-full border-b px-4 md:px-6 **:no-underline transition-all duration-300',
          // Remove the conflicting background classes and use scrolled state
          scrolled
            ? 'bg-accent-foreground/30 backdrop-blur-none border-gray-200'
        ...
        <div className="container mx-auto flex h-16 max-w-screen-2xl items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {isMobile && (
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    className="group h-9 w-9 hover:bg-accent"
                    variant="ghost"
                    size="icon"
                  >
                    <HamburgerIcon />
                                ...
                                {link.items?.map((item, itemIndex) => (
                                  <li key={itemIndex}>
                                    <button
                                      ...
                                      className="flex w-full items-center rounded-md px-3 py-2 text-sm font-medium transition-colors  hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground cursor-pointer no-underline"
                                    >
                                      {item.label}
                                    </button>
                                  </li>
            ...
            <div className="flex items-center gap-6">
              ...
              >
                ...
                <span className="hidden text-white font-bold text-xl sm:inline-block">TradeBull</span>
              </button>
              {/* Navigation menu */}
              {!isMobile && (
                  ...
                  <NavigationMenuList className="gap-1">
                    {navigationLinks.map((link, index) => (
                      <NavigationMenuItem key={index}>
                        {link.submenu ? (
                          <>
                            <NavigationMenuTrigger className="bg-transparent transition-none!  hover:bg-transparent data-state-open:bg-transparent">
                              {link.label}
                            </NavigationMenuTrigger>
                            <NavigationMenuContent>
          ...
          <div className="flex items-center gap-3">
            <Button
              variant="ghost"
              size="sm"
              className="text-sm font-medium px-4 h-9 text-white hover:bg-white/10 hover:text-white"
            ...
            <Button
              size="sm"
              onClick={onCtaClick}
              className="text-sm font-bold px-4 h-9 rounded-md shadow-sm bg-white text-zinc-600 hover:bg-white/90"
            >
              {ctaText}
            </Button>
          </div>
